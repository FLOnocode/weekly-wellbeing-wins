          ...
          return (
            <button
              ...
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                console.log("Icon clicked:", challenge.id);
                onToggle(challenge.id);
              }}
              className={cn(
                "relative p-3 rounded-xl transition-all duration-300 touch-manipulation",
                "bg-white border border-gray-200 shadow-lg hover:shadow-xl active:shadow-md",
                "h-12 w-12 flex items-center justify-center cursor-pointer select-none",
                "focus:outline-none focus:ring-2 focus:ring-wellness-400 focus:ring-offset-2",
                "hover:bg-gray-50 active:scale-95",
                "z-10 pointer-events-auto",
                isCompleted && "bg-wellness-100 border-wellness-300",
              )}
              type="button"
